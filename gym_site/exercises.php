...
<a href="new_exercise.php">Create Exercise</a>
        </div> 
</style>
</head>
<body>
...
$result = $conn->query($sql);
        ...
        if ($result !== FALSE){
            echo "<h2 style='font-size:200%'>PengCub Workouts</h2>";
            while ($row = $result->fetch_assoc()) {
				
                $field1name = $row['Exercise_Number'];
				$field2name = $row['Descriptor'];
				$field3name = $row['Exercise_Type'];
				$field4name = $row['Exercise_Name'];
                echo '<tr> '?>
							<td class="contact-delete">
							<form action='delete_exercise.php' method="post">
							<input type="hidden" name="Exercise_Name" value="<?php echo $row['Exercise_Name']; ?>">
