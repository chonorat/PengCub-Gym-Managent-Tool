...
$sql = "SELECT * from class_registration where Member_Number='$_POST[Member_Number]'";
$result = $conn->query($sql);

        if ($result !== FALSE){
            echo "<h2 style='font-size:200%'>Registered Classes</h2>";
            while ($row = $result->fetch_assoc()) {
				
                $field1name = $row['Member_Number'];
				$field2name = $row['Class_Reg_Date'];
				$field3name = $row['Guests'];
				$field4name = $row['Section_Number'];
                echo '<tr> 
                            <td>'.$field1name. '</td>
							<td>'.$field2name. '</td>
							<td>'.$field3name. '</td>
